...
local function worker(user_args)
    ...
    local function show_battery_warning()
		...
		-- Batería cargada
		local function full_battery()
				naughty.notify {
						text = "Ya puedes desconectar el cargador.",
						title = "Batería completamente cargada",
						timeout = 25,
            hover_timeout = 0.5,
            position = warning_msg_position,
            bg = "#0A2698",
    ...
    function(widget, stdout)
		...
		-- Para permitir dispositivos inalámbricos:
		for s in stdout:gmatch("[^\r\n]+") do
			bat_num = string.match(s,"Battery (%d): .+ capacity .+")
			if bat_num ~= nil then
				break
			end
		end
		...
		for s in stdout:gmatch("[^\r\n]+") do
            status, charge_str, _ = string.match(s, 'Battery ' .. bat_num .. ': (%a+), (%d?%d?%d)%%,?(.*)')
			if status ~= nil then
				break
			end
		end
        ...
        if (charge >= 0 and charge < 15) then
            ...
            if enable_battery_warning and status ~= 'Charging' and os.difftime(os.time(), last_battery_check) > 120 then
        ...
        elseif (charge >= 80 and charge < 100) then batteryType = "battery-full%s-symbolic"
				elseif charge == 100 then batteryType = "battery-full%s-symbolic"
